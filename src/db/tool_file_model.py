...
class ToolFileModel(Base):
    ...
    tool_id = Column(CHAR(36), ForeignKey('tools.id'), nullable=False, comment='关联工具ID')
    file_url = Column(String(512), nullable=False, comment='文件存储路径')
    tool_llm_content = Column(Text, comment='LLM生成的内容')
    create_time = Column(DateTime, server_default=func.now(), comment='创建时间')
    update_time = Column(DateTime, server_default=func.now(), onupdate=func.now(), comment='更新时间')

    # 关联到主工具表
    tool = relationship('ToolClassModel', back_populates='files')
