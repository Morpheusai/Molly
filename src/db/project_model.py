...
class ProjectModel(Base):
    ...
    project_type = Column(Enum('IIT', '注册试验', '探索性研究'), nullable=True, server_default=text("'IIT'"), comment='项目类型')
    indication = Column(String(128), comment='适应症')
    study_phase = Column(Enum('I期', 'II期', 'III期', '未指定', 'IIT'), nullable=True, server_default=text("'未指定'"), comment='研究分期')
    is_multicenter = Column(Enum('是', '否'), nullable=True, server_default=text("'否'"), comment='是否多中心')
    ...
    doctor_enrollment_count = Column(Integer, nullable=True, server_default=text("1"), comment='医生入组人数')
    patient_enrollment_count = Column(Integer, nullable=True, comment='患者入组人数')
    hospital = Column(String(128), nullable=True, comment='所属医院')
    phone = Column(String(32), nullable=True, comment='手机号')
    enrollment_criteria = Column(String(512), nullable=True, comment='入组要求')
