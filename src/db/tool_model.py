...
class ToolModel(Base):
    ...
    message_id = Column(CHAR(36), ForeignKey('message.id'), comment='关联的聊天记录ID')
    tool_id = Column(String(128), comment='工具id')
    tool_name = Column(String(128), comment='工具名称')
    tool_args = Column(String(255), comment='工具调用参数')
    tool_result = Column(String(255), comment='工具返回结果')
